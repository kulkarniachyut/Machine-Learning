			...
			for sgd_decay in sgd_decays:
				for sgd_mom in sgd_moms:
					...
					print('Score for architecture = '+ str(arch) + '  lambda = ' + str(reg_coeff) + ' decay =  ' + str(sgd_decay) +
						' momentum = '+ str(sgd_mom) +' actfn = {4}: {5}'.format(arch, reg_coeff, sgd_decay,
...
norm_X_tr ,norm_X_te =  normalize(X_tr, X_te)

total_time = time.time()
print " -------------  Part D  (1) -----------------"
archs = [[50,2],[50,50,2],[50,50,50,2],[50,50,50,50,2]]
testmodels(norm_X_tr, y_tr, norm_X_te, y_te,archs , 'relu', 'softmax', [0.0],30, 1000, 0.001, sgd_decays=[0.0], sgd_moms=[0.0],sgd_Nesterov=False, EStop=False, verbose=0)
...
print " -------------  Part F  -----------------"
start_time = time.time()
testmodels(norm_X_tr, y_tr, norm_X_te, y_te,archs , 'relu', 'softmax', [0.0],30, 1000, 0.0005, sgd_decays=[0.0], sgd_moms=[0.0],sgd_Nesterov=False, EStop=False, verbose=0)
print("--- %s seconds ---" % (time.time() - start_time))

print " -------------  Part G  -----------------"
archs = [[50,800,500,300,2]]

for i in [10**-7, 5*(10**-7),(10**-6),5*(10**-6),(10**-5)]:
	testmodels(norm_X_tr, y_tr, norm_X_te, y_te, archs, 'relu', 'softmax', [i], 30, 1000, 0.0005, sgd_decays=[0.0],
...
print " -------------  Part I  -----------------"
for i in [10**-5, 5*(10**-5),(10**-4),3*(10**-4),7*(10**-4),(10**-3)]:
	testmodels(norm_X_tr, y_tr, norm_X_te, y_te, archs, 'relu', 'softmax', [5*(10^-7)], 100, 1000, 0.00001, sgd_decays=[i],
			   sgd_moms=[0.0], sgd_Nesterov=False, EStop=False, verbose=0)

print " -------------  Part J  -----------------"
for i in [0.99, 0.98,0.95,0.9,0.85]:
        testmodels(norm_X_tr, y_tr, norm_X_te, y_te, archs, 'relu', 'softmax', [0.0], 50, 1000, 0.00001, sgd_decays=[5*(10**-5)],
                           sgd_moms=[i], sgd_Nesterov=True, EStop=False, verbose=0)

print " -------------  Part K  -----------------"
testmodels(norm_X_tr, y_tr, norm_X_te, y_te,archs , 'relu', 'softmax', [10**-7],100, 1000, 0.00001, sgd_decays=[5*(10**-5)], sgd_moms=[0.99],sgd_Nesterov=True, EStop=True, verbose=0)


print " -------------  Part L  -----------------"
archs = [[50,50,2],[50,500,2],[50,500,300,2],[50,800,500,300,2],[50,800,800,500,300,2]]
for i in [10**-7, 5*(10**-7),(10**-6),5*(10**-6),(10**-5)]:
	for j in [10**-5,5*(10**-5),(10**-4)]:
		testmodels(norm_X_tr, y_tr, norm_X_te, y_te, archs, 'relu', 'softmax', [i], 100, 1000, 0.00001, sgd_decays=[j],sgd_moms=[0.99], sgd_Nesterov=True, EStop=True, verbose=0)
