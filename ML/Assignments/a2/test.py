...
for i in comb:
    trainCombDf[i] = testDf2[i[0]]*testDf2[i[1]]


testCombDf = pd.concat([testDf2,testSquareDf,testCombDf],axis=1)
print len(testCombDf.ix[0])
# print trainCombDf
...
meanTest = testCombDf['Y'].mean()
stdTest = testCombDf['Y'].std()


testCombDf = testCombDf - testCombDf.mean()
testCombDf = testCombDf/testCombDf.std()
print testCombDf
...
finalParams1= getfinalParams(testCombDf)
print finalParams
...
for i in range(0, len(testCombDf)):
    testerrors.append(errorVal(testCombDf.ix[i], testDf.ix[i],meanTest,stdTest,finalParams1))

print 'mse :  ' , (np.sum(np.square(testerrors)))/len(testerrors)